...
export default function CategoryPage({ category, onProductClick }: CategoryPageProps) {
  ...
  const [weightRange, setWeightRange] = useState([0, 5000]);
  ...
  // Get unique categories from products for the filter sidebar
  const allCategories = useMemo(() => {
    const categories = [...new Set(products.map(p => p.category))];
    return categories.map(cat => ({
      name: cat === 'Fruit wine' ? 'Fruit Wine' : 
            cat === 'Frozen proteins' ? 'Frozen Proteins' :
            cat === 'Beauty & Personal Care' ? 'Beauty & Personal Care' : cat,
      category: cat
    }));
  }, []);
  ...
  const categoryDisplayNames: Record<string, string> = {
    ...
    'Vegetables & Fresh Produce': 'Vegetables & Fresh Produce',
  ...
  const filteredProducts = useMemo(() => {
    ...
    console.log('URL decoded category:', decodeURIComponent(category));
    
    // First, decode the URL parameter to handle spaces and special characters
    const decodedCategory = decodeURIComponent(category);
    
    let filtered = decodedCategory === 'all' 
      ? products 
      : products.filter(product => {
          // Use case-insensitive comparison with trimmed strings
          const matches = product.category.trim().toLowerCase() === decodedCategory.trim().toLowerCase();
          console.log(`Product: ${product.name}, Product Category: "${product.category}", Looking for: "${decodedCategory}", Matches: ${matches}`);
          return matches;
        });
    ...
    filtered = filtered.filter(product => {
      ...
      if (weightStr.includes('kg')) {
        ...
        if (kgMatch) {
          weightInGrams = parseFloat(kgMatch[1]) * 1000;
        }
      } else if (weightStr.includes('g')) {
        const gMatch = weightStr.match(/(\d+(\.\d+)?)\s*g/);
        if (gMatch) {
          weightInGrams = parseFloat(gMatch[1]);
        }
      } else if (weightStr.includes('ml')) {
        const mlMatch = weightStr.match(/(\d+(\.\d+)?)\s*ml/);
        if (mlMatch) {
          weightInGrams = parseFloat(mlMatch[1]);
        }
      } else if (weightStr.includes('l')) {
        const lMatch = weightStr.match(/(\d+(\.\d+)?)\s*l/);
        if (lMatch) {
          weightInGrams = parseFloat(lMatch[1]) * 1000;
        }
      }
      
      // If we couldn't parse weight, return true (don't filter out)
      if (weightInGrams === 0) return true;
  ...
  // Handle product card click - only for navigation
  ...
  // Handle view click - opens the modal
  ...
  const categoryTitle = categoryDisplayNames[decodeURIComponent(category).toLowerCase()] || decodeURIComponent(category);
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              ...
              {decodeURIComponent(category) !== 'all' && ` in ${categoryTitle}`}
            </p>
          </div>
              ...
              {filteredProducts.length === 0 && (
                <div className="text-center py-12 bg-white rounded-lg shadow-md">
                  ...
                  <p className="text-gray-600 mb-6">
                    {decodeURIComponent(category) === 'all' 
